...
for index in range(len(test_list)):
    ...
    elif value > expensive_item_price:
        expensive_item_price = value
        expensive_item = (test_list[index][0])

print(f"The average unit price is ${round(calc / number, 2)}\nThe cheapest "
      f"item is {cheapest_item} with the price of ${best_value_price} per unit"
      f"\nThe most expensive item is {expensive_item} with the price of "
      f"{expensive_item_price} per unit")
